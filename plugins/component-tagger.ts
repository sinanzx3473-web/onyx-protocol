        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // Also skip React Three Fiber (R3F) components to prevent crashes
              const skipElements = new Set([
                'html', 'head', 'body', 'script', 'style', 'meta', 'link', 'title',
                // R3F primitives and components - all lowercase for case-insensitive matching
                ...
                'hemispherelight', 'rectarealight', 'group', 'scene',
                'perspectivecamera', 'orthographiccamera', 'shadermaterial',
                'meshbasicmaterial', 'meshstandardmaterial', 'meshphysicalmaterial',
                'points', 'line', 'linesegments', 'sprite',
                // R3F drei components
                'orbitcontrols', 'suspense', 'stars', 'sky', 'environment',
                'contactshadows', 'float', 'text', 'html', 'center',
                // Three.js object types that shouldn't be tagged
                'object3d', 'buffergeometry', 'texture'
              ]);
              if (skipElements.has(tagName.toLowerCase())) return;
              
              // Skip files containing R3F/Three.js code entirely
